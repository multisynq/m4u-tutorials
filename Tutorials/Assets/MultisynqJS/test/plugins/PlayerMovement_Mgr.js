...
export class PlayerMovement_Mgr_Model extends Model {
  init(options) {
    ...
    this.isJumping         = new Map() // Track jumping state
  ...
  tick() {
    ...
    for (const [playerId, velocity] of this.velocities) {
      ...
      if (pos.y <= this.groundLevel) {
        ...
        this.isJumping.set(playerId, false)
      ...
      const isJumping = this.isJumping.get(playerId)

      // Send updates if moving and time elapsed, or if currently jumping
      if ((isMoving && timeSinceLastBroadcast >= this.broadcastInterval) || isJumping) {
  ...
  onPlayerJump(playerId) {
    ...
    this.isJumping.set(playerId, true)

    // Send immediate position update when jump starts
